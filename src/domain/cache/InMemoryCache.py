...
from domain.cache.IndexNotifiable import IndexNotifiable
...
class InMemoryCache(CacheBase):
    ...
    def remove(self, key):
            if key in self._cache:
                ...
                if key in self._last_index_values:
                    for field_name, old_value in self._last_index_values[key].items():
                        if field_name in self._indexes and old_value in self._indexes[field_name]:
                            del self._indexes[field_name][old_value]
                    
                    del self._last_index_values[key]
                
                if isinstance(obj, IndexNotifiable):
                    obj._caches.discard(self)
